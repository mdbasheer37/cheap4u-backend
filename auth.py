...
def _setup_paystack_sync(app, user_id, name, email, phone):
    """
    Create Paystack customer + DVA synchronously within app context.
    Called from background thread started by register().
    FIX: Uses passed app instead of current_app (which doesn't exist in threads).
    """
    with app.app_context():
        try:
            ...
            db.session.commit()
            app.logger.info(f'Paystack customer created for user {user_id}: {customer_code}')
...
def register():
    ...
    # FIX 1: Run Paystack setup in background thread WITH app reference
    # This fixes DVA not being created automatically
    import threading
    app = current_app._get_current_object()
    t = threading.Thread(
        target  = _setup_paystack_sync,
        args    = (app, user.id, name, email, phone),
        daemon  = True,
    )
    t.start()

    # FIX 2: Send OTP — return 200 success even if SMS fails
    # This fixes "Registration failed: Server error (HTTP 500)"
    sms_sent, _ = _send_otp(user, purpose='registration')

    if not sms_sent:
        current_app.logger.error(f'OTP SMS failed for {phone}')
        # CRITICAL FIX: was returning 500 which blocked the OTP screen
        # Now returns 200 so Kivy app proceeds to OTP verification screen
        return jsonify({
            'status':  'success',
...
def verify_otp():
    ...
    if datetime.utcnow() > otp.expires_at:
        return jsonify({'status': 'error',
                        'message': 'OTP expired. Request a new one.'}), 400
...
def login():
    ...
    # Try to create DVA in background if missing
    if not user.virtual_account_number:
        import threading
        app = current_app._get_current_object()
        t = threading.Thread(
            target = _setup_paystack_sync,
            args   = (app, user.id, user.name, user.email, user.phone),
            daemon = True,
        )
        t.start()
